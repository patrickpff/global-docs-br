...
export const AL_IE: IEStateValidator & IEStateMask = {
    validate(value: string): boolean {
        if (!value) return false;

        const digits = value.replace(/\D/g, "");

        // IE de AL tem 9 dígitos
        if (digits.length !== 9) return false;

        // Deve começar com 24
        if (!digits.startsWith("24")) return false;

        // Terceiro dígito deve ser 0, 1, 2 ou 3
        const typeDigit = digits[2];
        if (!["0", "3", "5", "7"].includes(typeDigit)) return false;
